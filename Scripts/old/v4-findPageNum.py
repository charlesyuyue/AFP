...
def extractAccountingPolicySection(path):
    ...
    extracted = ""
    foundSection = 0    # Bool to keep track whether the page number is found
    foundToT = 0
    foundTitle = 0  # Bool to keep track whether the keyword "policy" is found
    ifInSection = 0 # Bool to keep track whether currently appending the section
    targetSectinPageNum = 0
    pageCount = 1
    for page in PDFPage.get_pages(fp, pagenos, maxpages=maxpages,
                                  password=password,
                                  caching=caching,
                                  check_extractable=True):
        # if in the first 10 pages and we have not found Table of Contents
        if pageCount <= 10 and foundToT is not 1:
                ...
                if line.strip():
                    if foundToT is 1:
                        if "财务报告" in line:
                            ...
                            temp = re.sub("[^0-9]", "", line) # Remove non-numeric characters
                            targetSectinPageNum = int(temp)
                            break
                    elif "目录" in line:
                        foundToT = 1

        elif pageCount >= targetSectinPageNum:
            interpreter.process_page(page)
            # Get all text in the current page
            data = retstr.getvalue()
...
pdfDir = "C:/Users/William/Desktop/AFP/r/kangmei2015.pdf"
text = extractAccountingPolicySection(pdfDir)
...
pdfDir = "C:/Users/William/Desktop/AFP/r/kangmei2017.pdf"
text = extractAccountingPolicySection(pdfDir)
text = getChinese(text)
seg_list = jieba.cut(text)
seg_list = removeStopWords(seg_list)
dc3 = CountKey(seg_list)
...
seg_list = jieba.cut(text)
seg_list = removeStopWords(seg_list)
dc4 = CountKey(seg_list)

print(MergeKeys(dc1,dc2))
print(MergeKeys(dc1,dc3))
print(MergeKeys(dc1,dc4))

print(MergeKeys(dc2,dc3))
print(MergeKeys(dc2,dc4))

print(MergeKeys(dc3,dc4))